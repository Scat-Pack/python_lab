...
def add_student(name: str, grades: str = typer.Argument(..., help="Comma-separated grades (e.g., 90,85,88)")):
...
@app.command()
def add_course(name: str, student_grades: str = typer.Argument(..., help="Student grades as name: grade pairs (e.g., Alice:90,Bob:85)")):
    """Add a course with student grades."""
    grades_dict = {pair.split(":")[0]}:
    float(pair.split(":")[1]) for pair in student_grades.split(",")}
...
@app.command()
def update_student(name: str, grades: str = typer.Argument(..., help="Comma-seperated new grades (e.g., 95, 90, 88)")):
...
def view_all():
    ...
    for entity in entities:
        if entity._class__.__name__.lower() == entity_type.lower() and entity.name == name:
            avg = entity.calculator.grade()
...
def average(entity_type: str, name:str):
    ...
    entities = storage.load_all()
    for entity in entities:
        if entity.__class__.__name__.lower() == entity_type.lower() and entity.name == name:
            avg = entity.calculate_grade()
            typer.echo(f"Average grade for {entity_type} {name}: {avg:.2f}")
            return
    typer.echo(f"No {entity_type} named {name} found")

@app.command()
def export_summary(output: str = typer.Argument(..., help="Output file for summary")):
    """Export a summary of all students and courses to help file"""
    try:
        storage = GradeStorage()
